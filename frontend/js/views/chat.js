...
export function chatController(userId, username) {
	...
	if (startNewChatBtn) {
		startNewChatBtn.addEventListener('click', async () =>
			await handleStartNewChat(userId, username)
		);
	}
}
...
async function handleStartNewChat(currentUserId, currentUsername) {
	const targetUserInput = document.getElementById('targetUserInput');
	...
	await promptPrivateChat(currentUserId, chatInfo);
...
export async function launchTournamentChat(tournament_id) {
	...
	await promptPrivateChat(userData.id, chatInfo);
}
