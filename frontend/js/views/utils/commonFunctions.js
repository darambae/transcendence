...
export async function handleGame2Players(key, playerID, isAiGame) {
  console.log(adress);
  let url_sse = `https://${adress}:8443/server_pong/events?apikey=${key}&idplayer=${playerID}&ai=${isAiGame}`;
  let url_post = `https://${adress}:8443/server_pong/send-message`;
  let started = false;
  let game_stats;
  ctx.clearRect(0, 0, canvas.width, canvas.height)
  ...
  document.addEventListener('keydown', function(event) {
      switch(event.key) {
          ...
          case "q" :
              ...
              if (started == true) {
                fetch(`https://${adress}:8443/server_pong/forfait-game?apikey=${key}&idplayer=${playerID}`);
              }
              break;
          case "ArrowUp" : 
              if (playerID == 1) { 
...
export async function loadGamePlayable(apikey) {
  let isPlayable;
    ...
    await fetch(`https://${adress}:8443/server_pong/is-game-playable?apikey=${apikey}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
export async function setApiKeyWeb(apikey) {
  ...
  return fetch(`https://${adress}:8443/server_pong/set-api-key`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
