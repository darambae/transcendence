    ...
    document.addEventListener('keydown', function(event) {
        switch(event.key) {
            ...
            case "q" :
                ...
                if (started == true) {
                  fetch(`http://${adress}:8000/forfait-game?apikey=${key}&idplayer=${playerID}`); break;
                }
            case "ArrowUp" : 
                if (playerID == 1) { 
                    fetch(url_post, {
...
async function loadGamePlayable(apikey) {
  let isPlayable;
    ...
    await fetch(`http://${adress}:8000/is-game-playable?apikey=${apikey}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
async function setApiKeyWeb(apikey) {
  ...
  return fetch(`http://${adress}:8000/set-api-key`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
...
async function sendGameCreation(ctx, canvas) {
  let apiKey;
  await fetch(`http://${adress}:8000/get-api-key`)
    .then(response => {
      if (!response.ok) throw new Error("HTTP Error: " + response.status);
      return response.json();
    })
