...
def decodeJWT(request, encodedJwt=None) :
	...
	if res.status_code != 200 :
		...
		if (res_json.get('error') == "Token expired"):
			refresh_res = requests.get(f'https://access-postgresql:4000/api/token/refresh', headers={"Authorization" : f"bearer {encodedJwt}", 'Host': 'access-postgresql'}, verify=False)
