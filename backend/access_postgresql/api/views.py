	...
	def post(self, request) -> Response:
		...
		if data.get('chat_type') == 'tournament':
			...
			try:
				...
				if existing_chat:
					log_user_action('CHAT_GROUP_FOUND', current_user, chat_id=existing_chat.id, tournament_id=tournament_id)
					...
					chat_group.members.add(current_user)
					chat_group.save()
					log_user_action('CHAT_GROUP_CREATED', current_user, chat_id=chat_group.id, tournament_id=tournament_id)
		...
		if data.get('chat_type') == 'private':
			...
			try:
				...
				if existing_chat:
					...
					log_user_action('CHAT_GROUP_FOUND', current_user, chat_id=existing_chat.id, target_user_id=target_user.id)
					chat_group = existing_chat
				else:
					chat_group = ChatGroup.objects.create(name=chat_name)
					chat_group.members.add(current_user, target_user)
					chat_group.save()
					log_user_action('CHAT_GROUP_CREATED', current_user, chat_id=chat_group.id, target_user_id=target_user.id)
...
class ChatMessageView(APIView):
	...
	def post(self, request, group_id) -> Response:
		...
		try:
			...
			log_database_operation('MESSAGE_CREATED', 'Message', user_id=current_user.id, group_id=group_id, content_length=len(content))
